...
# inputs
# awsProxyUrl http://54.238.193.86:8080

def handler(context, inputs):
    awsProxyUrl = inputs.pop('awsProxyUrl') # get proxy url
    payload = { # check property and create payload
        'bucketName': inputs['bucketName'],
        'awsAccessKey': inputs['awsAccessKey'],
        'awsSecretKey': inputs['awsSecretKey']
    }
    res = requests.post(awsProxyUrl + '/bucketcreate', json=payload)
